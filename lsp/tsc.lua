...
local function supported_tsc(root_dir)
  local candidates = {}
  if root_dir then
    table.insert(candidates, vim.fs.joinpath(root_dir, 'node_modules/.bin/tsc'))
  end
  table.insert(candidates, vim.fn.exepath('tsc'))

  for _, candidate in ipairs(candidates) do
    if candidate ~= '' and vim.fn.executable(candidate) == 1 then
      local version = vim.fn.systemlist({ candidate, '--version' })[1] or ''
      local major = tonumber(version:match('Version%s+(%d+)'))
...
return {
  cmd = function(dispatchers, config)
    local cmd = supported_tsc((config or {}).root_dir)
    if cmd then
  ...
  root_dir = function(bufnr, on_dir)
    ...
    if supported_tsc(root_dir) then
      on_dir(root_dir)
    end
  end,
  settings = {
